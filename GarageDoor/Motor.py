...
class Motor(tk.Label):
    ...
    def moveSteps(self, direction, max_count, min_count):
        ...
        if (self._cycle >= max_count and direction == 1) or (
            self._cycle <= min_count and direction == -1
        ):
            self._direction = 0
        ...
        self._rotation_id = self.after(
            1, self.moveSteps, self._direction, max_count, min_count
        )
