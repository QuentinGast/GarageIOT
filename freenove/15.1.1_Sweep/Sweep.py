...
SERVO_MAX_DUTY = 92.5 + OFFSET_DUTY    # define pulse duty cycle for maximum angle of servo
...
def loop():
    while True:
        for angle in range(0, 361, 1):   # make servo rotate from 0 to 180 deg
            servoWrite(angle)
            time.sleep(SERVO_DELAY_SEC)
        time.sleep(0.5)
        for angle in range(360, -1, -1): # make servo rotate from 180 to 0 deg
            servoWrite(angle)
            time.sleep(SERVO_DELAY_SEC)
        time.sleep(0.5)
